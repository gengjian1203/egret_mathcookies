...
class Game extends egret.DisplayObjectContainer
{
    ...
    // 背景盘子对象
    private bgPanzi:egret.Bitmap;
    // 饼干群对象
    private group:GroupCooky;

    // 游戏得分
    private strGameScore:egret.TextField;
    private nGameScore:number;
    // 游戏时间
    private strGameTime:egret.TextField;
    private nGameTime:number;
    // 准备字符
    private strReadyGo:egret.TextField;
    //剩余时间定时器
...
//        window.location.href = "http://shop33649836.m.taobao.com";
    ...
    private LeftTimerFunc()
    {
        ...
        if(this.nGameTime <= 0)
        {
            ...
            var scene:MathCookys.GameOver = new MathCookys.GameOver(this.nGameScore);
            this.addChild(scene);
        }
        else
        {
